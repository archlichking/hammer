...
func writeBaseString(w io.Writer, method string, urlStr string, params url.Values) {
	...
	for _, kv := range p {
		...
		//log.Print(string(kv.key), string(encodedEqual), string(kv.value))
	}
}
...
func (c *Client) SignParam(credentials *Credentials, method, urlStr string, params url.Values) {
	...
	if credentials != nil {
		...
		params.Set("oauth_token", c.Credentials.Token)
	}
	params.Set("oauth_signature", signature(&c.Credentials, credentials, method, urlStr, params))
}

// Added by Rui
// SignParamWithBodyHash adds an OAuth signature to params See
// http://tools.ietf.org/html/rfc5849#section-3.5.2 for information about
// transmitting OAuth parameters in a request body and
// http://tools.ietf.org/html/rfc5849#section-3.5.2 for information about
// transmitting OAuth parameters in a query string.
func (c *Client) SignParamWithBodyHash(credentials *Credentials, method, urlStr string, params url.Values, body string) {
	params.Set("oauth_consumer_key", c.Credentials.Token)
	params.Set("oauth_signature_method", "HMAC-SHA1")
	params.Set("oauth_timestamp", strconv.FormatInt(time.Now().Unix(), 10))
	params.Set("oauth_version", "1.0")
	...
	// to create body_hash
	// req['oauth_body_hash'] != base64.b64encode(hashlib.sha1(request.data).digest()):

	h := sha1.New()
	io.WriteString(h, body)

	encodedSum := make([]byte, base64.StdEncoding.EncodedLen(len(h.Sum(nil))))

	base64.StdEncoding.Encode(encodedSum, h.Sum(nil))
	params.Set("oauth_body_hash", string(encodedSum))
...
func (c *Client) AuthorizationHeaderWithBodyHash(credentials *Credentials, method, urlStr string, params url.Values, body string) string {
	...
	c.SignParamWithBodyHash(credentials, method, urlStr, p, body)
	...
	buf.WriteString(`", oauth_body_hash="`)
	buf.Write(encode(p["oauth_body_hash"][0], false))
